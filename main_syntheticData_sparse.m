...
for gamma_idx = 1: length(gamma_list)
    ...
    for multiple_idx = 1:length(multiple_list)
        for j = 1:length(density_list)
            ...
            itersocp = zeros(len_m,1); iterLTR = zeros(len_m,1); iterRTR = zeros(len_m,1);iterLTR2 = zeros(len_m,1); iterRTR2 = zeros(len_m,1);
            for idx = 1: len_m
                % read data
                ...
                itersocp(idx) = Infosocp.iter; iterLTR(idx) = InfoLTR.iter;iterLTR2(idx) = InfoLTR2.iter;
            end
            % compare result
